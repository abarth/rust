...
impl Command {
    ...
    unsafe fn do_exec(&mut self, stdio: ChildPipes, maybe_envp: Option<&CStringArray>)
        ...
        let mut actions: [fdio_spawn_action_t; 3];

        // Clone stdin, stdout, and stderr
        if let Some(fd) = stdio.stdin.fd() {
            actions[0].action = FDIO_SPAWN_ACTION_TRANSFER_FD;
            actions[0].local_fd = fd;
        } else {
            actions[0].action = FDIO_SPAWN_ACTION_CLONE_FD;
            actions[0].local_fd = 0;
        }
        actions[0].target_fd = 0;

        if let Some(fd) = stdio.stdout.fd() {
            actions[1].action = FDIO_SPAWN_ACTION_TRANSFER_FD;
            actions[1].local_fd = fd;
        } else {
            actions[1].action = FDIO_SPAWN_ACTION_CLONE_FD;
            actions[1].local_fd = 1;
        }
        actions[1].target_fd = 1;

        if let Some(fd) = stdio.stderr.fd() {
            actions[2].action = FDIO_SPAWN_ACTION_TRANSFER_FD;
            actions[2].local_fd = fd;
        } else {
            actions[2].action = FDIO_SPAWN_ACTION_CLONE_FD;
            actions[2].local_fd = 2;
        }
        actions[2].target_fd = 2;
        ...
        zx_cvt(fdio_spawn_etc(0, FDIO_SPAWN_SHARE_JOB | FDIO_SPAWN_CLONE_LDSVC | FDIO_SPAWN_CLONE_NAMESPACE,
                              self.get_argv()[0], self.get_argv().as_ptr(), envp, 3, actions.as_ptr(),
                              &mut process_handle, ptr::null()))?;
